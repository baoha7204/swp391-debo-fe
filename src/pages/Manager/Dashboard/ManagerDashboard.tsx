...
const ManagerDashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async (branchId: number) => {
            try {
                const treatmentRes = await axios.get(`${API_ENDPOINTS.DASHBOARD.BRANCH_TREATMENT}/${branchId}`);
                const categoryRes = await axios.get(`${API_ENDPOINTS.DASHBOARD.BRANCH_CATEGORY}/${branchId}`);
                const employeeRes = await axios.get(`${API_ENDPOINTS.USERS.EMPLOYEE_WITH_BRANCH_ID}/${branchId}`);
    ...
    const data = [
        ...
        },
        {
            ...
            component: <TreatmentPieChart treatmentCategories={treatmentCategories} />,
            flex: 3,
        },
    ];
