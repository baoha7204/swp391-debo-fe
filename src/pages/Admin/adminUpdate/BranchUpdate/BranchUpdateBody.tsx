...
function BranchUpdateBody() {
  const [handleSubmit, isSubmitting, control, setValues, onUpload] =
    useBranchUpdate();
  ...
  useEffect(() => {
    if (branchesList || managers.length > 0) {
      const isCurrentManagerAvailable = managers.some(
        (manager) => manager.id === branchesList.mngId
      );
  ...
  return (
    <Box component="form" noValidate onSubmit={handleSubmit}>
      <Grid container className="create-screen" spacing={10}>
        ...
        <Grid item xs={false} sm={false} md={6}>
          <FormImagePicker name="avt" control={control} onUpload={onUpload} />
        </Grid>
      </Grid>
      <MyButton
        sx={{
